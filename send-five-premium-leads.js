...
console.log('--- 📧 BLINQ OUTBOX DISPATCHER ---');
...
function buildPremiumHtml(bodyText, businessName, city) {
  ...
  const pricingText = isColombia ? '$50 USD (o 200.000 COP)' : '$50 USD';
    ...
    <div class="container">
      ...
      <div class="content">
        ...
        <div class="pricing-box">
          ...
          <div class="pricing-badge">Cero Anticipo · Entrega en 48h</div>
        </div>
      </div>
      <div class="footer">
...
async function sendOutbox() {
  ...
  console.log(`\n⏳ Pipelining outbox dispatch for exactly ${targetLeadIds.length} premium leads...`);
  ...
  for (const id of targetLeadIds) {
    ...
    const draft = db.drafts.find(d => d.lead_id === id);

    if (!lead || !draft) {
      console.log(`⚠️ Missing lead or draft data for id: ${id}. Skipping.`);
      continue;
    }

    if (draft.status === 'sent') {
      console.log(`✔ Lead already contact in history: "${lead.business_name}" (${lead.email}). Skipping.`);
      continue;
    }
    ...
    try {
      ...
      lead.crm_notes = `[Auto Outbox] Contactado con copywriting conversacional y precios regionales el ${new Date().toLocaleDateString()}. Resend ID: ${result.id}\n${lead.crm_notes || ''}`;
  ...
  console.log(`🚀 All updates written to local DB fallback! Run or visit the Next.js Dashboard to trigger Vercel sync.`);
}
